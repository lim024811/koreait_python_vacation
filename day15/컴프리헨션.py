...
# [연산식 for 변수 in 리스트 (if 조건-필터조건)] -> 앞의 연산식은 for 문에서 사용하는 것
...
# 홀수는 그대로 두고, 짝수만 제곱
# 데이터의 전체를 조작 - [연산식(삼항연산자) for 변수 in 리스트]
# 삼항연산자 : True값 if 조건 else False 값
...
# {} 컴프리헨션 - 조건문(필터링) -> {연산식 for문 if문}
scores = {"철수" : 85, "영희" : 42, "민수" : 90, "지우" : 80}

# {"파이썬고-이름": 점수} 60이상만
# items()  -> [("철수", 85),...] dict는 리스트가 아니니깐 리스트처럼 만들어줌
...
menus = {"사과": 2000, "빵": 1500,"귤": 800}
# menus에 있는 가격들을 *0.9 해서 10%할인시킨
# 결과 출력! {"사과": 1800...}
...
class Student:
    ...
    # __repr__(): 다른 자료형에 담겨있을 때 출력값을 리턴
    def __repr__(self):
