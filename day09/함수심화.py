# 매개변수의 기본값 설정
def hello(name, nationalty="대한민국"):
    print(f"[{nationalty}] {name}님 안녕하세요")
...
# def hello(nationalty="대한민국", name):
#     print(f"[{nationalty}] {name}님 안녕하세요")

# 파이썬은 매개변수를 들어오는 데이터의 순서로 기억
# 기본값설정이 되어있는게 나중 순서로 와야 명확하게 데이터를 할당 가능하다.
...
# 매개변수명은 args임
def args_fx(*args):
    # 매개변수의 갯수와 무관하게 모두 tuple 하나로 포장해서 들어온다.
...
other_data = {"name": "김길동", "age": 30, "address": "부산"}

# kwargs 언패킹 -> 이미 dict로 포장되어있으니, 또 포장하지마셈
